                                ...
                                <div id="priceD" class="accordion-collapse collapse show">
                                    <div class="accordion-body">
                                        ...
                                        <div class="track-wrap">
                                            ...
                                            <input type="range" class="range-input" min="0"
                                                max="{{ $maxPrice }}" value="0" id="priceRangeMin">
                                            <input type="range" class="range-input" min="0"
                                                max="{{ $maxPrice }}" value="{{ $maxPrice }}"
                                                id="priceRangeMax">
                                        </div>
                                        <div class="range-values">
                                            ...
                                            <span id="priceMaxLabel">${{ $maxPrice }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    // ── Dual Range Slider (defined OUTSIDE iife so it's available globally) ──
    function initDualRangeSlider(minRangeId, maxRangeId, minNumId, maxNumId, minLabelId, maxLabelId) {
        ...
        if (minNum) {
            minNum.addEventListener("input", function() {
                var v = Math.min(Math.max(parseFloat(this.value) || MIN, MIN), +maxRange.value - GAP);
                minRange.value = v;
                update();
            });
        }
        ...
        if (maxNum) {
            maxNum.addEventListener("input", function() {
                var v = Math.max(Math.min(parseFloat(this.value) || MAX, MAX), +minRange.value + GAP);
                maxRange.value = v;
                update();
            });
        }
    ...
    (function() {
        ...
        clearBtn?.addEventListener("click", function() {
            ...
            if (maxLabel && maxEl) maxLabel.textContent = "$" + parseFloat(maxEl.max).toFixed(2);
        ...
        initDualRangeSlider(
            ...
            "price_min_input", "price_max_input",
