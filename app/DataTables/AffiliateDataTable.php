...
class AffiliateDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('user_name', function ($row) {
                return $row->affiliateUser
                    ? $row->affiliateUser->first_name . ' ' . $row->affiliateUser->last_name
                    : 'N/A';
            })
    ...
    <div class="d-flex align-items-center gap-1">
        ...
        <button class="btn btn-sm btn-outline-secondary copy-btn"
            ...
            ->addColumn('action', function ($row) {
                return '
               <div class="d-flex ">
                <a href="' . route('admin.affiliates.edit', $row->id) . '"
                    class="btn btn-sm btn-primary" title="Edit">
                    <i class="fa fa-edit"></i>
                </a>
                 <button type="button" class="btn btn-danger btn-sm deleteAffiliateUser"  data-id="' . encrypt($row->id) . '">
            ...
            //  Sorting - raw SQL string only, NO closures
            ...
            //  Searching - filterColumn closures are fine (not serialized)
            ->filterColumn('user_name', function ($query, $keyword) {
                $query->where(function ($q) use ($keyword) {
                    $q->where('affiliate_users.first_name', 'like', "%{$keyword}%")
                        ->orWhere('affiliate_users.last_name', 'like', "%{$keyword}%");
                });
            })
            ->filterColumn('email', function ($query, $keyword) {
                $query->where('affiliate_users.email', 'like', "%{$keyword}%");
            })
            ->filterColumn('referral_code', function ($query, $keyword) {
                $query->where('affiliates.referral_code', 'like', "%{$keyword}%");
            })

            ->rawColumns(['action', 'status', 'referral_link'])
            ->setRowId('id');
    }

    public function query(Affiliate $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
            ...
            ->ajax([
                'url' => route('admin.affiliates.list'),
                'type' => 'GET',
            ])
            ->orderBy(1, 'asc')
            ->selectStyleSingle()
